package com.metmit.simulation.handler.xpath.parser;
...
import com.metmit.simulation.handler.xpath.exception.NoSuchAxisException;
...
import com.metmit.simulation.handler.xpath.model.Predicate;
        ...
        TAG {
            ...
            public void parse(XpathStateMachine stateMachine) throws XpathSyntaxErrorException {
                ...
                if (stateMachine.tokenQueue.peek() == '*') {
                    ...
                    stateMachine.xpathChain.getLast().setSelectParams(ListUtils.newArrayList("*"));
                ...
                if (stateMachine.tokenQueue.matches("@")) {// 遇到属性抽取动作
                    ...
                    if (stateMachine.tokenQueue.peek() == '*') {
                        ...
                        stateMachine.xpathChain.getLast()
                                .setSelectParams(ListUtils.newArrayList(attributeKey));
                    }
                    stateMachine.state = PREDICATE;
                    stateMachine.tokenQueue.consumeWhitespace();
                    return;
                ...
                stateMachine.xpathChain.getLast()
                        .setSelectFunction(FunctionEnv.getSelectFunction("tag"));
                stateMachine.xpathChain.getLast().setSelectParams(ListUtils.newArrayList(tagName));
                stateMachine.state = PREDICATE;
            }
        },
        PREDICATE {
